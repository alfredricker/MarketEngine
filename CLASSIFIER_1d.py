...
df = pd.read_csv('csv_data/TRAINING-9-2.csv')
...
checkpoint_callback = ModelCheckpoint(
    dirpath="CHECKPOINTS",
    filename="best-checkpoint-9-2.ckpt",
    save_top_k=1,
    verbose=True,
    monitor="val_loss",
    mode="min"
...
def train_model():
    ...
    torch.save(model.state_dict(),'CHECKPOINTS/CLASSIFIER-9-2.ckpt')
    ...
    data = {
        ...
        'Predicted-Probs': predicted_probs_numpy
    }

    df_comparison = pd.DataFrame(data)
    df_comparison.to_csv('csv_tests/comparison_classifier-9-2.csv')


#train_model()


sym_list = ['AKAM','AES','BYND','BAC','CMCSA','FITB','FLEX','IP','LSXMK','MTCH','PLUG','RKT','T','UBER','VICI','VTRS',
            'SMPL','VZ','DAL','HAL','PFE','ET','HLIT','FTAI','UCTT','XPOF','APLD','NOG','WSC']

sym_list_b = ['A','AA','ABBV','ABNB','ABT','ACHR','ADM','LCID','SOUN','QS','SOFI','LUV','AI','ENVX','NYCB','MU',
            'NCLH','KEY','SIRI','BMY','OPEN','LYFT','KDP','RIOT','CVNA','U','UPST','NOVA','FCEL']

sym_list_c = ['SBUX','RUN',
    'TDOC', 'FUBO', 'RCL', 'RTX', 'PCG', 'EBAY', 'GM', 'SPCE',
    'JBLU', 'BA', 'GPS', 'CROX', 'IONQ', 'WU', 'ARR', 'EW', 'MXL',
    'COIN', 'USB', 'SNAP', 'OSTK', 'HBAN', 'AR', 'FSR', 'CHPT', 'FCX',
    'HTZ', 'KMI', 'BSX'
]

sym_list_d = ['MRVL', 'WBD',
    'CSX', 'PYPL', 'JNJ', 'RBLX','BAX', 'ET', 'NOV', 'DISH', 'LUMN', 'KO', 'SPWR', 'PEP', 'LAZR', 'MAT',
    'HOOD', 'AMAT', 'WBA', 'EYE', 'JPM', 'FHN', 'CARR',
    'SCHW', 'ESRT', 'PARA', 'PINS', 'PTEN'
]


device = 'cpu'

checkpoint = torch.load('CHECKPOINTS/CLASSIFIER-9-2.ckpt')
...
def preprocess_multiple_eval_data(sym_list):
    def sym_pth(sym):
        pth = f'csv_data/{sym}-2023-09-02.csv'
        return pth
    df_list = []
...
def stock_pth(symbol):
    pth = f'csv_data/{symbol}-2023-09-03.csv'
    return pth

trading_dict = {'Ticker':[],'Predicted':[],'PredictedProbs':[]}

for sym in sym_list_c:
    pth = stock_pth(sym)
...
df_eval.to_csv('csv_data/EVAL-9-2-4.csv')
...
df_eval.to_csv('csv_data/EVAL-9-2.csv')
'''